#!/usr/bin/env python3
"""
Import dangerous goods from CSV with duplicate handling
"""
import os
import sys
import csv
...
def import_dangerous_goods(csv_file_path):
    ...
    try:
        ...
        with open(csv_file_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                try:
                    ...
                    # Skip if no UN number or duplicate
                    if not un_number or not proper_shipping_name:
                        continue
                    
                    if un_number in seen_un_numbers:
                        print(f"Skipping duplicate UN {un_number}: {proper_shipping_name}")
                        continue
                    
                    seen_un_numbers.add(un_number)
        ...
        # Show sample data
        cursor.execute("SELECT un_number, proper_shipping_name, hazard_class FROM dangerous_goods_dangerousgood LIMIT 5;")
        sample_data = cursor.fetchall()
        print("\nSample imported data:")
        for record in sample_data:
            print(f"  UN{record[0]}: {record[1]} (Class {record[2]})")
