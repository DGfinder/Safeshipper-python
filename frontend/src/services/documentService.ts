...
class DocumentService {
  ...
  async generateDGTransportDocument(request: DocumentRequest): Promise<DocumentResponse> {
    try {
      const response = await fetch(`${this.baseUrl}/documents/generate-dg-transport/`, {
        method: 'POST',
        headers: {
  ...
  async collectSDSDocuments(dangerousGoods: DocumentRequest['dangerousGoods']): Promise<SDSDocument[]> {
    try {
      const response = await fetch(`${this.baseUrl}/sds/collect-for-shipment/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async collectEPGDocuments(dangerousGoods: DocumentRequest['dangerousGoods']): Promise<EPGDocument[]> {
    try {
      const response = await fetch(`${this.baseUrl}/epg/collect-for-hazard-classes/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async generateCompletePackage(request: DocumentRequest): Promise<DocumentResponse> {
    try {
      const response = await fetch(`${this.baseUrl}/documents/generate-complete-package/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  private async generateMockCompletePackage(request: DocumentRequest): Promise<DocumentResponse> {
  ...
  getDocumentTypeDescription(type: string): string {
    const descriptions = {
      'DG_TRANSPORT': 'Dangerous Goods Transport Document - Official declaration for transport compliance',
      'SDS_COLLECTION': 'Safety Data Sheets - Chemical safety information for all dangerous goods',
      'EPG_PACKAGE': 'Emergency Procedure Guides - Emergency response procedures by hazard class',
      'COMPLETE_PACKAGE': 'Complete Documentation Package - All required transport documents in one bundle'
    };
    return descriptions[type as keyof typeof descriptions] || 'Document package';
  }

  validateDocumentRequest(request: DocumentRequest): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    request.dangerousGoods.forEach((dg, index) => {
      if (!dg.un || !dg.class || !dg.properShippingName) {
        errors.push(`Dangerous good ${index + 1} is missing required information`);
      }
    });
