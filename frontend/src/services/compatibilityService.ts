...
class CompatibilityService {
  ...
  async checkCompatibility(items: DangerousGood[]): Promise<CompatibilityResult> {
    try {
      const response = await fetch(`${this.baseUrl}/dangerous-goods/check-compatibility/`, {
        method: 'POST',
        headers: {
  ...
  async checkPairCompatibility(item1: DangerousGood, item2: DangerousGood): Promise<CompatibilityCheck> {
    try {
      const response = await fetch(`${this.baseUrl}/dangerous-goods/check-pair-compatibility/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  private transformBackendResponse(data: any): CompatibilityResult {
    return {
      ...
      issues: data.incompatible_pairs?.map((pair: any) => ({
  ...
  private getMockCompatibilityResult(items: DangerousGood[]): CompatibilityResult {
    ...
    for (let i = 0; i < items.length; i++) {
      for (let j = i + 1; j < items.length; j++) {
        ...
        if ((item1.class.startsWith('1') && item2.class === '5.1') ||
          ...
          issues.push({
            ...
            segregationCode: 'X',
            rule: 'IMDG 7.2.2.3',
            explanation: 'Explosives must be segregated from oxidizing substances'
          });
        }

        // Example: Class 3 (Flammable liquids) needs separation from Class 5.1 (Oxidizers)
        if ((item1.class === '3' && item2.class === '5.1') ||
            (item2.class === '3' && item1.class === '5.1')) {
          warnings.push(`Flammable liquids (${item1.class === '3' ? item1.properShippingName : item2.properShippingName}) require separation from oxidizers`);
        }
      }
    }

    if (issues.length === 0 && warnings.length === 0) {
      recommendations.push('All dangerous goods are compatible for transport together');
    } else if (issues.length > 0) {
      recommendations.push('Remove incompatible items or transport in separate vehicles');
    } else if (warnings.length > 0) {
      recommendations.push('Ensure proper segregation distances are maintained during loading');
    }
  ...
  private getMockPairCompatibility(item1: DangerousGood, item2: DangerousGood): CompatibilityCheck {
    ...
    for (const pair of incompatiblePairs) {
      if ((pair.classes.includes(item1.class) && pair.classes.includes(item2.class)) &&
        ...
        return {
          ...
          explanation: pair.code === 'X' ? 'These substances must not be transported together' : 
