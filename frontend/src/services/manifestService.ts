...
class ManifestService {
  ...
  async uploadAndAnalyzeManifest(request: ManifestAnalysisRequest): Promise<ManifestAnalysisResponse> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/manifests/upload-and-analyze/`, {
        method: 'POST',
        body: formData,
      });
  ...
  async checkProcessingStatus(documentId: string): Promise<ProcessingStatus> {
    ...
    try {
      const response = await fetch(`${this.baseUrl}/manifests/${documentId}/status/`);
  ...
  private async simulateManifestAnalysis(request: ManifestAnalysisRequest): Promise<ManifestAnalysisResponse> {
    ...
    return {
      ...
      results: {
        ...
        recommendations: this.generateRecommendations(mockResults.dangerousGoods)
      }
    };
  }
      ...
      dangerousGoods: [
        {
          id: '1',
          un: '1170',
          properShippingName: 'Ethanol or Ethyl alcohol or Ethanol solutions or Ethyl alcohol solutions',
          class: '3',
          subHazard: undefined,
          packingGroup: 'II',
          quantity: '1000 L',
  ...
  private generateWarnings(dangerousGoods: any[]): string[] {
    ...
    if (dangerousGoods.some(dg => dg.confidence < 0.8)) {
      warnings.push('Some dangerous goods were detected with low confidence. Manual verification recommended.');
    }
    
    if (dangerousGoods.some(dg => dg.class === '3')) {
      warnings.push('Flammable liquids detected. Ensure proper ventilation and fire safety measures.');
    }
    
    if (dangerousGoods.length > 5) {
      warnings.push('Multiple dangerous goods detected. Additional compatibility checking recommended.');
    }
  ...
  private generateRecommendations(dangerousGoods: any[]): string[] {
    ...
    recommendations.push('Review all detected dangerous goods for accuracy before finalizing manifest.');
    
    if (dangerousGoods.length > 1) {
      recommendations.push('Run compatibility analysis to ensure safe transport of combined dangerous goods.');
    }
    
    recommendations.push('Collect Safety Data Sheets (SDS) for all dangerous goods before transport.');
    recommendations.push('Verify packaging groups and quantities match physical shipment.');
  ...
  private transformResults(results: any) {
    return {
      dangerousGoods: results.dangerous_goods?.map((dg: any) => ({
...
export type { ManifestAnalysisRequest, ManifestAnalysisResponse, ProcessingStatus };