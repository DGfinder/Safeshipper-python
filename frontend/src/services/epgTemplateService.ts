...
class EPGTemplateService {
  ...
  async generateEPGFromSDS(request: EPGGenerationRequest): Promise<EPGGenerationResult> {
    try {
      // First try backend API
      return await this.generateViaAPI(request);
    } catch (error) {
      console.error('API EPG generation failed, falling back to simulation:', error);
      // Fallback to local generation
      return this.simulateEPGGeneration(request);
    }
  }

  private async generateViaAPI(request: EPGGenerationRequest): Promise<EPGGenerationResult> {
  ...
  private async simulateEPGGeneration(request: EPGGenerationRequest): Promise<EPGGenerationResult> {
  ...
  private extractEPGDataFromSDS(request: EPGGenerationRequest): EPGData {
    ...
    return {
      ...
      reactivity: sdsData.extractedData.physicalProperties?.flammability || 'See SDS for reactivity information',
      ...
      // Emergency Contacts
      emergencyContacts: this.getStandardEmergencyContacts(request.locationInfo),
      ...
      documentVersion: '1.0',
      regulatoryReferences: ['ADG Code', 'IMDG Code', 'Workplace Health and Safety Regulations']
    };
  }
  ...
  private extractSpecialHazards(sdsData: SDSExtractionResult): string[] {
    ...
    if (sdsData.extractedData.physicalProperties?.flammability?.toLowerCase().includes('flammable')) {
      hazards.push('Flammable - keep away from heat, sparks, open flames');
    }
    
    if (sdsData.extractedData.transportInfo?.transportHazardClass?.includes('5.1')) {
      hazards.push('Oxidizing agent - incompatible with combustible materials, reducing agents');
    }
    
    if (sdsData.extractedData.transportInfo?.transportHazardClass?.includes('8')) {
  ...
  private extractEmergencyProcedures(sdsData: SDSExtractionResult): EPGEmergencyProcedure[] {
    const procedures: EPGEmergencyProcedure[] = [];

    // Fire emergency procedures
    procedures.push({
      scenario: 'Fire involving this material',
      immediateActions: 'Evacuate area. Alert fire brigade. Do not allow water runoff to reach storm drains. Use appropriate extinguishing agents. Cool containers with water spray from safe distance.',
      category: 'fire',
      severity: 'high'
    });
    ...
    procedures.push({
      ...
      immediateActions: 'Evacuate area. Prevent entry to storm drains. Contain spill using non-combustible absorbent material. Collect in suitable containers for disposal. Clean area with water if safe to do so.',
      category: 'spill',
      severity: 'medium'
    });
    ...
    procedures.push({
      ...
      immediateActions: 'Remove from exposure. Move to fresh air immediately. Keep warm and at rest. Give artificial respiration if breathing has stopped. Obtain medical attention.',
      category: 'exposure',
      severity: 'critical'
    });
    ...
    procedures.push({
      ...
      immediateActions: 'Stop vehicle safely. Alert emergency services. Identify dangerous goods using placard/documentation. Keep public away. Contact emergency number on transport documents.',
      category: 'transport',
      severity: 'high'
    });
  ...
  private extractFirstAidProcedures(sdsData: SDSExtractionResult): EPGFirstAidProcedure[] {
    ...
    if (firstAid?.eyeContact) {
      procedures.push({
        ...
        symptoms: firstAid.immediateSymptoms?.filter(s => s.toLowerCase().includes('eye')) || ['Eye irritation', 'Redness', 'Tearing'],
        immediateActions: ['Flush with water for 15+ minutes', 'Remove contact lenses', 'Seek medical attention']
      });
    }
    ...
    if (firstAid?.inhalation) {
      procedures.push({
        ...
        symptoms: firstAid.immediateSymptoms?.filter(s => s.toLowerCase().includes('respir')) || ['Coughing', 'Shortness of breath', 'Chest tightness'],
        immediateActions: ['Move to fresh air', 'Monitor breathing', 'Give oxygen if needed', 'Seek medical attention']
      });
    }
    ...
    if (firstAid?.skinContact) {
      procedures.push({
        ...
        symptoms: firstAid.immediateSymptoms?.filter(s => s.toLowerCase().includes('skin')) || ['Skin irritation', 'Redness', 'Burns'],
        immediateActions: ['Remove contaminated clothing', 'Flush with water', 'Seek medical attention if severe']
      });
    }
    ...
    if (firstAid?.ingestion) {
      procedures.push({
        ...
        immediateActions: ['Do not induce vomiting', 'Rinse mouth', 'Seek medical attention immediately']
      });
    }
    ...
    if (procedures.length === 0) {
      return [
        {
          exposureType: 'eyes',
          procedure: 'Immediately flush eyes with plenty of water for at least 15 minutes. Remove contact lenses if easily removable. Seek medical attention.',
          symptoms: ['Eye irritation', 'Redness', 'Tearing'],
          immediateActions: ['Flush with water', 'Remove contact lenses', 'Seek medical attention']
        },
        {
          exposureType: 'inhalation',
          procedure: 'Remove from exposure. Move to fresh air immediately. Monitor breathing. Seek medical attention if symptoms persist.',
          symptoms: ['Coughing', 'Shortness of breath'],
          immediateActions: ['Move to fresh air', 'Monitor breathing', 'Seek medical attention']
        },
        {
          exposureType: 'skin',
          procedure: 'Remove contaminated clothing. Wash skin with plenty of water. Seek medical attention if irritation persists.',
          symptoms: ['Skin irritation', 'Redness'],
          immediateActions: ['Remove clothing', 'Flush with water', 'Monitor for irritation']
        },
        {
          exposureType: 'ingestion',
          procedure: 'Do not induce vomiting. Rinse mouth with water. Give water to drink if conscious. Seek immediate medical attention.',
          symptoms: ['Nausea', 'Vomiting', 'Abdominal pain'],
          immediateActions: ['Do not induce vomiting', 'Rinse mouth', 'Seek medical attention']
        }
      ];
    }
  ...
  private getStandardEmergencyContacts(locationInfo?: { country: string; region: string }): EPGContactInfo[] {
  ...
  private extractExtinguishingMedia(sdsData: SDSExtractionResult): string[] {
    ...
    if (fireFighting?.extinguishingMedia && fireFighting.extinguishingMedia.length > 0) {
      return fireFighting.extinguishingMedia;
    }
    
    // Fallback: Based on hazard class, provide appropriate extinguishing media
    const hazardClass = sdsData.extractedData.transportInfo?.transportHazardClass || '';
    
    if (hazardClass.includes('5.1')) {
      return ['Water spray', 'Foam', 'Dry chemical', 'Carbon dioxide'];
    } else if (hazardClass.includes('3')) {
      return ['Foam', 'Dry chemical', 'Carbon dioxide', 'Water spray to cool containers'];
  ...
  private extractSpillResponse(sdsData: SDSExtractionResult): string[] {
    ...
    if (spillData) {
      ...
      if (spillData.personalPrecautions) {
        responses.push(`Personal precautions: ${spillData.personalPrecautions}`);
      }
      ...
      if (spillData.containmentMethods && spillData.containmentMethods.length > 0) {
  ...
  private generateWarnings(request: EPGGenerationRequest): string[] {
    ...
    if (!request.sdsData.extractedData.transportInfo) {
      warnings.push('Limited transport information available - verify UN number and hazard class');
    }
    
    if (request.sdsData.processingMetadata.confidence < 0.8) {
      warnings.push('SDS extraction confidence is below 80% - manual review recommended');
    }
    
    if (request.sdsData.warnings.length > 0) {
      warnings.push('SDS processing warnings detected - review source document');
    }
  ...
  private transformAPIResponse(data: any): EPGData {
    return {
      ...
      emergencyProcedures: data.emergency_procedures?.map((proc: any) => ({
        scenario: proc.scenario,
        immediateActions: proc.immediate_actions,
        category: proc.category,
        severity: proc.severity
      })) || [],
      firstAidProcedures: data.first_aid_procedures?.map((proc: any) => ({
  ...
  validateEPGRequest(request: EPGGenerationRequest): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
