...
export default function ShipmentsPage() {
  ...
  const displayResults = isSemanticSearch && searchResults.length > 0 ? 
    searchResults : 
      ...
      <div className="p-6 space-y-6">
          ...
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {isSemanticSearch ? <Brain className="h-5 w-5 text-blue-600" /> : <Filter className="h-5 w-5" />}
              ...
              {semanticAnalysis && isSemanticSearch && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium text-blue-800">AI Understanding</span>
                  </div>
                ...
                <>
                  ...
                  {semanticAnalysis && (
                    <span className="ml-2 text-blue-600">
                      • {semanticAnalysis.intent === 'compliance_check' ? 'Compliance focused' : 'General search'}
                    </span>
                  )}
                </>
              ) : (
        ...
        <div className="grid gap-6">
          {displayResults.map((result) => {
            ...
            const isSearchResult = result.type === 'shipment' && 'relevanceScore' in result;
            const shipmentData = isSearchResult ? result.metadata : result;
            
            return (
            <Card key={result.id} className={`hover:shadow-md transition-shadow ${
              isSearchResult && isSemanticSearch ? 'border-l-4 border-l-blue-400' : ''
            }`}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {isSearchResult && isSemanticSearch && (
                      <div className="text-xs text-blue-600">
                        Search relevance: {Math.round(result.relevanceScore * 100)}%
                      </div>
                    )}
                  </div>
                </div>
