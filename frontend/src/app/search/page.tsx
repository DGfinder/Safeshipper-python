...
function SearchPageContent() {
  ...
  const getResultCounts = () => {
    const counts = {
      ...
      compliance_record: searchResults.filter(r => r.type === 'compliance_record').length
    };
    return counts;
  };
  ...
  const getResultIcon = (type: string) => {
    switch (type) {
      ...
      case 'vehicle': return <Truck className="h-4 w-4 text-orange-500" />;
      case 'user': return <Users className="h-4 w-4 text-purple-500" />;
      case 'compliance_record': return <Shield className="h-4 w-4 text-red-500" />;
  ...
  const getResultColorClass = (type: string) => {
    switch (type) {
      case 'shipment': return 'border-l-blue-400 bg-blue-50/30';
      case 'document': return 'border-l-green-400 bg-green-50/30';
      case 'vehicle': return 'border-l-orange-400 bg-orange-50/30';
      case 'user': return 'border-l-purple-400 bg-purple-50/30';
      case 'compliance_record': return 'border-l-red-400 bg-red-50/30';
  ...
  const formatResultMetadata = (result: SearchResult) => {
    ...
    if (metadata.status) {
      items.push({ label: 'Status', value: metadata.status, icon: AlertTriangle });
    }
    if (metadata.route) {
    ...
    if (metadata.dangerousGoods && metadata.dangerousGoods.length > 0) {
      const classes = metadata.dangerousGoods.map((dg: any) => dg.class).join(', ');
          ...
          <CardContent>
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="grid w-full grid-cols-6">
                ...
                <TabsTrigger value="compliance_record" className="flex items-center gap-1">
                            ...
                            <div className="flex-1">
                              ...
                              {formatResultMetadata(result).length > 0 && (
                                <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                                  {formatResultMetadata(result).map((item, idx) => (
