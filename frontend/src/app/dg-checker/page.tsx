...
export default function DGCheckerPage() {
  ...
  const { 
    ...
    error: searchError 
  } = useSearchDangerousGoods(debouncedSearchTerm, debouncedSearchTerm.length >= 2);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Dangerous Goods Checker</h1>
            <p className="text-gray-600 mt-1">
              Build your shipment load and check for dangerous goods compatibility
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="h-fit">
            <CardHeader>
              ...
              <CardDescription>
                Search by UN number or proper shipping name to add items to your load
              </CardDescription>
            </CardHeader>
            <CardContent>
              {/* Search Input */}
              ...
              <div className="mt-6">
                ...
                {selectedDGs.length === 0 ? (
                  <div className="text-center py-8 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg">
                    ...
                    <p className="text-sm">Search and select items to build your load</p>
                  </div>
                ) : (
              ...
              {selectedDGs.length < 2 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">
                    You need at least 2 dangerous goods items to perform a compatibility check
                  </p>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className={`text-lg font-semibold ${
                          compatibilityData.is_compatible ? 'text-green-900' : 'text-red-900'
                        }`}>
                          {compatibilityData.is_compatible 
                            ? 'Load is Compliant' 
                        ...
                        <p className={`text-sm ${
                          compatibilityData.is_compatible ? 'text-green-700' : 'text-red-700'
                        }`}>
                          {compatibilityData.is_compatible
                  ...
                  {/* Conflict Details */}
                  {!compatibilityData.is_compatible && compatibilityData.conflicts.length > 0 && (
                    <div className="space-y-3">
                      <h4 className="font-medium text-gray-900">Conflict Details:</h4>
                  ...
                  <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                    <h4 className="font-medium text-gray-900 mb-2">Load Summary</h4>
                    <div className="text-sm text-gray-600 space-y-1">
                      <div>Total Items: {selectedDGs.length}</div>
                      <div>
                        Hazard Classes: {
                          [...new Set(selectedDGs.map(dg => dg.hazard_class.split('.')[0]))].join(', ')
                        }
                      </div>
                      <div>
                        Status: {compatibilityData.is_compatible ? '✅ Ready for transport' : '❌ Requires review'}
                      </div>
                    </div>
                  </div>
                </div>
