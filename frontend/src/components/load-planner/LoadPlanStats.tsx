...
export function LoadPlanStats({ loadPlan, className }: LoadPlanStatsProps) {
  ...
  const dgClassCounts = placed_items.reduce((acc, item) => {
  ...
  const totalWeight = placed_items.reduce((sum, item) => sum + item.weight_kg, 0);
  const totalVolume = placed_items.reduce((sum, item) => 
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{efficiency_stats.total_items}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{efficiency_stats.remaining_capacity.toLocaleString()}</p>
                <p className="text-xs text-gray-500">kg</p>
              </div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p><span className="font-medium">Max Weight:</span> {vehicle.dimensions.max_weight_kg.toLocaleString()} kg</p>
            </div>
            <div>
              <p><span className="font-medium">Dimensions:</span> {vehicle.dimensions.length}×{vehicle.dimensions.width}×{vehicle.dimensions.height}m</p>
              <p><span className="font-medium">Total Volume:</span> {(vehicle.dimensions.length * vehicle.dimensions.width * vehicle.dimensions.height).toFixed(1)} m³</p>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {Object.entries(dgClassCounts).map(([dgClass, count]) => (
                <div key={dgClass} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                  <span className="text-sm font-medium">
                    {dgClass === 'GENERAL' ? 'General Cargo' : `Class ${dgClass}`}
                  </span>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2 text-sm text-gray-600">
            <p>• All {efficiency_stats.total_items} items successfully placed using 3D bin packing optimization</p>
            <p>• Load distribution optimized for vehicle stability and weight limits</p>
            <p>• Dangerous goods compatibility rules applied during placement</p>
            <p>• {efficiency_stats.remaining_capacity > 1000 ? 'Significant' : 'Limited'} remaining capacity available for additional items</p>
          </div>
        </CardContent>
      </Card>
    </div>
