...
export function FleetMap({ vehicles, onVehicleSelect, className }: FleetMapProps) {
  // Filter vehicles that have location data
  const vehiclesWithLocation = useMemo(() => 
    vehicles.filter(vehicle => vehicle.location && vehicle.location.lat && vehicle.location.lng),
    [vehicles]
  );
  ...
  const mapCenter = useMemo(() => {
    ...
    const avgLat = vehiclesWithLocation.reduce((sum, v) => sum + v.location!.lat, 0) / vehiclesWithLocation.length;
    const avgLng = vehiclesWithLocation.reduce((sum, v) => sum + v.location!.lng, 0) / vehiclesWithLocation.length;
      ...
      <Card className={className}>
        ...
        <CardContent>
          <Alert>
            ...
            <AlertDescription>
              No vehicles found. Vehicles will appear here when they are assigned to shipments or have recent location updates.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <Alert className="border-yellow-200 bg-yellow-50">
            ...
            <AlertDescription className="text-yellow-800">
              {vehicles.length} vehicle(s) found, but none have location data available.
              Vehicles need to update their GPS position to appear on the map.
            </AlertDescription>
          </Alert>
          
          <div className="mt-4 space-y-2">
            <h4 className="font-medium text-sm text-gray-700">Vehicles without location:</h4>
            {vehicles.slice(0, 5).map(vehicle => (
              <div key={vehicle.id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                <span className="font-mono text-sm">{vehicle.registration_number}</span>
            ...
            {vehicles.length > 5 && (
              <p className="text-xs text-gray-500">... and {vehicles.length - 5} more</p>
            )}
          </div>
        </CardContent>
      </Card>
                  ...
                  <div className="min-w-64 space-y-3">
                    ...
                    {vehicle.assigned_driver && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="font-medium">{vehicle.assigned_driver.name}</span>
                      </div>
                    )}
                    ...
                    {vehicle.active_shipment && (
                      <div className="border-t pt-2 space-y-1">
                        ...
                        <div className="text-sm text-gray-600 space-y-1">
                          <p><span className="font-medium">Tracking:</span> {vehicle.active_shipment.tracking_number}</p>
                          <p><span className="font-medium">Customer:</span> {vehicle.active_shipment.customer_name}</p>
                          <p><span className="font-medium">Route:</span> {vehicle.active_shipment.origin_location} → {vehicle.active_shipment.destination_location}</p>
                          {vehicle.active_shipment.estimated_delivery_date && (
                            <div className="flex items-center gap-1">
                              <Clock className="h-3 w-3 text-gray-400" />
                              <span>ETA: {new Date(vehicle.active_shipment.estimated_delivery_date).toLocaleDateString()}</span>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="border-t pt-2 text-xs text-gray-500 space-y-1">
                      ...
                      {vehicle.company && (
                        <p><span className="font-medium">Company:</span> {vehicle.company.name}</p>
                      )}
