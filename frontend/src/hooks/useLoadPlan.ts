...
const createMockLoadPlan = (shipmentId: string, vehicleIds: string[]): LoadPlan => {
  ...
  const totalVolume = vehicleDimensions.length * vehicleDimensions.width * vehicleDimensions.height;
  const usedVolume = mockItems.reduce((sum, item) => 
...
// Mock API functions
async function generateLoadPlan(shipmentId: string, vehicleIds: string[]): Promise<LoadPlan> {
  // Simulate API delay
...
async function saveLoadPlan(shipmentId: string, loadPlan: LoadPlan): Promise<{ message: string; load_plan_id: string }> {
  // Simulate API delay
...
export function useGenerateLoadPlan() {
  return useMutation({
    mutationFn: ({ shipmentId, vehicleIds }: { shipmentId: string; vehicleIds: string[] }) => {
...
export function useSaveLoadPlan() {
  return useMutation({
    mutationFn: ({ shipmentId, loadPlan }: { shipmentId: string; loadPlan: LoadPlan }) => {
