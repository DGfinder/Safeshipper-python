// components/auth/customer-auth-guard.tsx
// Authentication guard specifically for customer portal access
...
function CustomerLogin({ onSuccess }: CustomerLoginProps) {
...
export function CustomerAuthGuard({ children, fallback }: CustomerAuthGuardProps) {
  const { isAuthenticated, user, isHydrated } = useAuthStore();
  const { data: customerAccess, isLoading: accessLoading } = useCustomerAccess();
  ...
  if (showLogin || !isAuthenticated) {
    return fallback || <CustomerLogin onSuccess={() => setShowLogin(false)} />;
  }
  ...
  // User is authenticated but doesn't have customer access
  if (!customerAccess) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <Card className="max-w-md w-full">
          ...
          <CardContent className="text-center space-y-4">
            <p className="text-gray-600">
              Your account does not have customer portal access. Please contact your administrator.
            </p>
            <Button 
  ...
  // User is authenticated and has customer access
  return <>{children}</>;
}